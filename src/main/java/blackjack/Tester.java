...
//  }
  ...
  public static void openMenu() { //this method literally opens the menu.
    ...
    OptionsComponent beginningComponent = new OptionsComponent(); //we initialize an blackjack.OptionsComponent which is the JComponent that contains the properties (buttons, titles, etc.) of the menu.
  ...
  public static Thread gameRefreshThread = new Thread () { //this first thread (each thread must have a run method) serves to continuously [since we put true inside the while loop, it will continue forever.] refresh the component.
    public void run () {
      while(true){
        newGame.atmosphereComponent.refresh(currentBalance, playerScore, dealerScore-1, newGame.faceDown); //this line calls the refresh method of the blackjack.GameComponent atmosphere component which is declared inside the Game class. This method updates the score and balance values.
